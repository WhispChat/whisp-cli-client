    ...
    while (std::getline(iss, part, (char)23)) {
      ...
      if (any.Is<client::Message>()) {
        ...
        if (user_msg.has_registered_user()) {
          ClientWindow::add_message("[" + user_msg.registered_user().username() + "]: " + user_msg.content());
          std::cout << "[" << user_msg.registered_user().username()
                    << "]: " << user_msg.content() << '\n';
        } else if (user_msg.has_guest_user()) {
          ClientWindow::add_message("[" + user_msg.guest_user().username() + "(guest)]: " + user_msg.content());
